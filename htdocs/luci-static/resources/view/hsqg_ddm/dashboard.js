...
return view.extend({
	...
	render: function(data) {
		...
		if (window.localStorage) {
			var savedUnit = window.localStorage.getItem('hsqg_unit_system');
			if (savedUnit) self.unitSystem = savedUnit;
			
			// Instant cache snapshot if live status is pending
			if (!initialStatus || !initialStatus.ddm) {
				try {
					var cached = JSON.parse(window.localStorage.getItem('hsqg_last_telemetry') || '{}');
		...
		var topBar = E('div', { class: 'hw-top-bar' }, [
				...
				E('div', { class: 'hw-unit-group' }, [
					E('button', {
						...
						click: function() {
							...
							if (window.localStorage) window.localStorage.setItem('hsqg_unit_system', 'dual');
							updateUnitButtons();
					...
					E('button', {
						...
						click: function() {
							...
							if (window.localStorage) window.localStorage.setItem('hsqg_unit_system', 'metric');
							updateUnitButtons();
							if (self.lastData) updateDashboard(self.lastData);
						}
					}, _('Metric (°C, dBm)')),
					E('button', {
						...
						click: function() {
							...
							if (window.localStorage) window.localStorage.setItem('hsqg_unit_system', 'imperial');
							updateUnitButtons();
							if (self.lastData) updateDashboard(self.lastData);
						}
					}, _('Imperial (°F, µW)'))
			...
			E('div', { class: 'hw-top-right' }, [
				E('span', { style: 'font-size: 0.82em; opacity: 0.75; font-family: ui-monospace, monospace; font-weight: 600;' },
					_('Module: ') + (initialStatus.host || '192.168.150.1')
				)
			])
		]);
		...
		var updateDashboard = function(res) {
			...
			if (rxStats) {
				...
				rxStats.appendChild(E('div', { class: 'hw-stat-row' }, [
					...
					E('span', { class: 'hw-stat-value' }, '-8.0 to -27.0 dBm')
				]));
				rxStats.appendChild(E('div', { class: 'hw-stat-row' }, [
			...
			if (onuStats) {
				...
				onuStats.appendChild(E('div', { class: 'hw-stat-row' }, [
					...
					E('span', { class: 'hw-stat-value' }, dev.lan_status || 'Up, 1000 Mbps')
				]));
			}
